...
export default ({ navigation }) => {
    ...
    async function loadInvoices() {
        setLoading(true);
        const [status, response] = await Requests.GET(`group/invoices?id=${groupId}`, user.jwt);
        setLoading(false);
        if (status == 200) {
    ...
    useEffect(() => {
        setInvoices(allInvoices.filter(invoice => {
            ...
            return includesLower(invoice.fullname, search) || `${invoice.amount}`.includes(search);
        }))
